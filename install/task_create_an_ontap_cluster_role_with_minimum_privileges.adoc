...
summary: Sie sollten eine ONTAP Clusterrolle mit minimalen Berechtigungen erstellen, damit Sie die ONTAP Administratorrolle nicht verwenden müssen, um Vorgänge in SnapCenter auszuführen. 
---
...
. Erstellen Sie einen Benutzer und weisen Sie diesem Benutzer die Rolle zu.
+
